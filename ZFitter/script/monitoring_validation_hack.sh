...
if [ -n "$ETA" ];then
    ...
    regionFile=data/regions/systematics_LC.dat 
    xVar=absEta
    if [ -n "$STEP4" ];then
...
if [ -n "$SLIDES" ];then
	...
	./script/makeValidationSlides.sh --dirData ${dirData} --selection ${selection} --invMass_var ${invMass_var} --dirMC ${dirMC} --commonCut $commonCut --type validation --tag=$TAG $period #--rereco ${rereco}
    fi
