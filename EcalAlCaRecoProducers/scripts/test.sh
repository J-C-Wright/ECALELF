...
if [ ! -f "$mySingleERecoFilePath" ]; then
    ...
    cmsDriver.py myreco -s RAW2DIGI,RECO -n $num  --filein="file:$myRawSingleEFileLocal" --data --conditions=$GT --era=Run2_2016  --scenario=pp --processName=reRECO --customise=L1Trigger/Configuration/customiseReEmul.L1TEventSetupForHF1x1TPs --dirout=$singleDir
else
    echo "$mySingleERecoFilePath exists"
fi
if [ ! -f "$myDoubleEGRecoFilePath" ]; then
    echo "myDoubleEGRecoFilePath doesn't exist"
    cmsDriver.py myreco -s RAW2DIGI,RECO -n $num  --filein="file:$myRawDoubleEGFileLocal" --data --conditions=$GT --era=Run2_2016  --scenario=pp --processName=reRECO --customise=L1Trigger/Configuration/customiseReEmul.L1TEventSetupForHF1x1TPs --dirout=$doubleDir
else
...
if [ ! -f "$myUncalWFilePath" ]; then
    ...
    cmsDriver.py myreco -s ALCA:EcalUncalWElectron -n $num --data --filein="file:$mySingleERecoFilePath" --conditions=$GT --secondfilein="file:$myRawSingleEFile" --dirout=$singleDir --era=Run2_2016  --scenario=pp --processName=alcaRaw --customise=L1Trigger/Configuration/customiseReEmul.L1TEventSetupForHF1x1TPs 
else
    echo "$myUncalWFilePath exists"
fi
if [ ! -f "$myUncalZFilePath" ]; then
    echo "$myUncalZFilePath doesn't exist"
    cmsDriver.py myreco -s ALCA:EcalUncalZElectron -n $num --data --filein="file:$myDoubleEGRecoFilePath" --conditions=$GT --secondfilein="file:$myRawDoubleEGFile" --dirout=$doubleDir --era=Run2_2016  --scenario=pp --processName=alcaRaw --customise=L1Trigger/Configuration/customiseReEmul.L1TEventSetupForHF1x1TPs 
else 
...
if [ ! -f "$myCalWFilePath" ]; then
    ...
    cmsDriver.py myreco -s ALCA:EcalCalWElectron -n $num --data --filein="file:$mySingleERecoFilePath" --conditions=$GT --secondfilein="file:$myRawSingleEFile" --dirout=$singleDir --era=Run2_2016  --scenario=pp --processName=alcaReco --customise=L1Trigger/Configuration/customiseReEmul.L1TEventSetupForHF1x1TPs 
else
    echo "$myCalWFilePath exists"
fi
if [ ! -f "$myCalZFilePath" ]; then
    echo "$myCalZFilePath doesn't exist"
    cmsDriver.py myreco -s ALCA:EcalCalZElectron -n $num --data --filein="file:$myDoubleEGRecoFilePath" --conditions=$GT --secondfilein="file:$myRawDoubleEGFile" --dirout=$doubleDir --era=Run2_2016  --scenario=pp --processName=alcaReco --customise=L1Trigger/Configuration/customiseReEmul.L1TEventSetupForHF1x1TPs 
else
...
if [ ! -f "$myRecalWFilePath" ]; then
    ...
    cmsDriver.py myreco -s ALCA:EcalRecalElectron -n $num --data --conditions=$GT --nThreads=4 --filein="file:$myUncalWFilePath" --dirout=$singleDir --customise_commands="process.options=cms.untracked.PSet(wantSummary=cms.untracked.bool(True))"   --process=RERECO --customise Calibration/EcalAlCaRecoProducers/customRereco.EcalRecal --era=Run2_2016  --scenario=pp --customise=L1Trigger/Configuration/customiseReEmul.L1TEventSetupForHF1x1TPs 
else
    echo "$myRecalWFilePath exists"
fi
if [ ! -f "$myRecalZFilePath" ]; then
    echo "$myRecalZFilePath doesn't exist"
    cmsDriver.py myreco -s ALCA:EcalRecalElectron -n $num --data --conditions=$GT --nThreads=4 --filein="file:$myUncalZFilePath" --dirout=$doubleDir --customise_commands="process.options=cms.untracked.PSet(wantSummary=cms.untracked.bool(True))"   --process=RERECO --customise Calibration/EcalAlCaRecoProducers/customRereco.EcalRecal --era=Run2_2016  --scenario=pp --customise=L1Trigger/Configuration/customiseReEmul.L1TEventSetupForHF1x1TPs 
else
    echo "$myRecalZFilePath exists"
fi

echo "[INFO] NTuples from ALCARECO"
cmsRun python/alcaSkimming.py isCrab=0 skim=WSkim maxEvents=$num type=ALCARECO files="file:$myCalWFilePath" doTree=3 tagFile=config/reRecoTags/test80x.py doTreeOnly=1 
#cmsRun python/alcaSkimming.py isCrab=0 skim=ZSkim maxEvents=$num type=ALCARECO files="file:$myCalZFilePath" doTree=3 tagFile=config/reRecoTags/test80x.py doTreeOnly=1 
